...
\begin{figure}[H]
  ...
  \caption{
  Bootstrap distributions of the mean distance to the future for simulated populations.
  Empirical distances to the future were sampled with replacement and mean values for each bootstrap sample were calculated across n=10,000 bootstrap iterations.
  The horizontal gray line indicates the median of the bootstrap distribution for the naive model.
  Each model is annotated by the p-value from a bootstrap hypothesis test between the mean distances for that model and the naive model (see Methods).
  }
...
\begin{figure}[H]
  ...
  \caption{
  Bootstrap distributions of the mean distance to the future for natural populations.
  Empirical distances to the future were sampled with replacement and mean values for each bootstrap sample were calculated across n=10,000 bootstrap iterations.
  The horizontal gray line indicates the median of the bootstrap distribution for the naive model.
  Each model is annotated by the p-value from a bootstrap hypothesis test between the mean distances for that model and the naive model (see Methods).
  }
