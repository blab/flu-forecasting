...
%% Insert summary statistics of future diversity without describing average Hamming distance
%% and remove approximation of future diversity text.
...
\begin{figure*}[ht]
  ...
  b) The percentile rank in observed distance to the future of the model's estimated best strain for 18 test timepoints.
  The estimated best strain is in the top 20th percentile of observed closest strains for 89\% of timepoints, confirming that the model makes a good choice when forced to select a single representative strain for the future population.
  }
...
We observed a growth rate correlation of $R = 0.38$~$(p < 0.001)$ and clade growth and decline prediction accuracies of 82\% and 53\%, respectively (Supplemental Fig.~\ref{fig:test_of_best_model_for_simulated_populations}A).
The estimated best strain was in the top 2nd percentile of observed closest strains for half of the test timepoints and in the top 20th percentile for 16 of 18 (89\%) of timepoints (Supplemental Fig.~\ref{fig:test_of_best_model_for_simulated_populations}B).
...
LBI was the best individual fitness metric by average distance to the future (Fig.~\ref{fig:unadjusted_model_accuracy_and_coefficients_for_natural_populations}) and tied HI cross-immunity and mutational load for the most timepoints outperforming the naive model of 17 (74\%) (Table~\ref{table_natural_model_selection}).
...
Specifically, we fit models based on HI cross-immunity and mutational load, LBI and mutational load, and all three of these metrics together.
...
The composite model of mutational load and LBI effectively captured clade growth and decline with 87\% and 89\% accuracy, respectively (Fig.~\ref{fig:validation_of_best_model_for_natural_populations}A).
...
Finally, we tested the performance of this same best model on out-of-sample data collected from October 1, 2015 through October 1, 2019.
...
\begin{figure*}[ht]
  ...
  \caption{
    ...
    The estimated closest strain to the future was identified by either the best model in the validation period (mutational load and LBI) or the best model in the test period (titer cross-immunity and mutational load).
  }
...
Our combined mutational load and LBI model favored the growth of the currently circulating clade A1b/197R, the maintenance of A1b/137F, and the decline of 3c3.A (Fig.~\ref{fig:nextstrain_forecasts}).
