...
We estimated the frequency of virus strains every six months using kernel density estimation (KDE) to represent the frequency of each strain.
...
Mutational load is the simplest of the functional constraint metrics, measuring the number of putatively deleterious mutations that have accumulated in each strain since their ancestor in the previous season.
...
\begin{table}[htb]
  \begin{center}
    ...
    \caption{
      ...
      Distances annotated with asterisks (*) were significantly closer to the future than the naive model as measured by bootstrap hypothesis tests (see Methods).
...
\begin{figure}[p]
  ...
  \jhc{Add and reference supplementary figure with naive model results to compare with these.}
  }
...
These results point to one aspect that often overlooked when comparing the genetic make-up of an asexual population at two time points:
The future population is unlikely to descend from any of the sampled tips but ancestral lineages of the future population merge with those of the present population in the past.
...
\subsubsection*{Model comparison by bootstrap hypothesis testing}

We compared the performance of each biologically-informed model to the corresponding population's naive model using bootstrap hypothesis tests \cite{EfronBradley1998}.
For each model's empirical distribution of earth mover's distances to the future, $\mathbf{y}$, and the corresponding naive model's distribution, $\mathbf{x}$, we applied the following procedure to test whether their means were significantly different.
We calculated the test statistic $t$ as follows where $n$ and $m$ were the respective lengths of the naive and biologically-informed model's distributions.

$$
t = \frac{\bar{x}-\bar{y}}{\sqrt{\sigma_x^2/n + \sigma_y^2/m}}
$$

Next, we combined the two empirical distributions of distances into a single distribution, $\mathbf{z}$, and calculated the mean of this distribution.
For each original distribution, we created a new distribution that was recentered on the mean of $\mathbf{z}$.
For example, we recentered the $\mathbf{x}$ distribution as $\mathbf{x^{'}} = \mathbf{x} - \bar{x} + \bar{z}$.
We drew $n$ and $m$ samples, respectively, from the resulting distributions, $\mathbf{x^{'}}$ and $\mathbf{y^{'}}$, with replacement for 10,000 bootstrap iterations.
For each iteration, we recalculated the test statistic $t^{'}$ for the means and variances of the bootstrap samples.
We calculated the p-value as the proportion of bootstrap samples for which the $t^{'}$ statistic was greater than or equal to the observed $t$ for the empirical distributions.
To assist with the interpretation of these p-values, we further bootstrapped each model's empirical distance distribution 10,000 times, calculated the mean of each bootstrap sample, and visualized the distributions of each model annotated by its p-value from the test above (Supplemental Figs.~\ref{sup_fig:bootstrap_distributions_for_simulated_sample_3} and \ref{sup_fig:bootstrap_distributions_for_natural_sample_1_with_90_vpm_sliding}).
