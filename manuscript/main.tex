...
To estimate antigenic drift with genetic data, we implemented the cross-immunity metrics originally defined by {\L}uksza and L\"assig \cite{Luksza:2014hj}.
...
For each individual strain at each timepoint, we estimated that strain's ability to escape cross-immunity by summing the exponentially-scaled epitope distances between previously circulating strains and the given strain as follows.
...
This method effectively estimated the antigenic drift associated with each branch in the phylogeny.
...
Finally, we calculated each strain's ability to escape cross-immunity by applying the same equation as above for epitope mutations where $D_{\mathrm{ep}}(a_{i}, a_{j})$ was replaced by the pairwise antigenic distance from the tree.
