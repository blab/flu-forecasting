...
The World Health Organization (WHO) Global Influenza Surveillance and Response System (GISRS) selects vaccine viruses to match circulating viruses, but because the process of vaccine development and distribution requires several months to complete, optimal vaccine strain selection requires an accurate prediction of which viruses will predominate approximately one year after vaccine viruses are selected.
...
\begin{figure*}[ht]
  ...
  \caption{
    ...
    The estimated future population at timepoint $u$, $\mathbf{\hat{x}}(u)$, is projected to the right with strains scaled in size by their projected frequency based on the true fitness model.
    ...
    Model coefficients minimize the earth mover's distance between amino acid sequences in the observed and estimated future populations across all training windows.
  }
  \label{fig:model}
  \end{center}
\end{figure*}
...
While epitope antigenic novelty was not predictive on its own, the consistently positive coefficient it received in the composite model suggested that it was an informative metric when effects of purifying selection were also represented in the model.
...
By itself, this linear fitness metric did not outperform the antigenic novelty metric (Table~\ref{table_simulated_model_selection}).
However, when we fit a composite model of the epitope ancestor and mutational load metrics, this model outperformed both the individual mutational load model and the composite antigenic novelty and mutational load model (Supplemental Fig.~\ref{sup_fig:unadjusted_composite_model_accuracy_and_coefficients_for_simulated_populations}).
...
\begin{figure*}[p]
  ...
  A) The correlation of log estimated clade growth ratio, $\log_{10}{\frac{\hat{x}(t + \Delta{t})}{x(t)}}$, and log observed clade growth ratio, $\log_{10}{\frac{x(t + \Delta{t})}{x(t)}}$, shows the model's ability to capture clade-level dynamics without explicitly optimizing for clade frequency targets.
...
First, we measured the ability of the true fitness model to accurately estimate dynamics of large clades (initial frequency $>15\%$) by comparing observed and estimated clade growth ratios.
The model's estimated growth ratios correlated well with observed growth ratios (Pearson's $R^2 = 0.52$, Supplemental Fig.~\ref{sup_fig:validation_of_best_model_for_simulated_populations}A).
...
Note, that the numbers of test and validation data points are small and we therefore expect fluctuations.
...
While observed and estimated growth ratios correlated more weakly for test data (Pearson's $R^2 = 0.14$), the accuracies of clade growth and decline predictions remained similar at 82\% and 53\%, respectively (Fig.~\ref{fig:test_of_best_model_for_simulated_populations}A).
...
We held out strains collected between October 1, 2015 and October 1, 2019 for model testing (Supplemental Fig.~\ref{sup_fig:cross_validation_for_natural_populations}).
...
HI antigenic novelty estimated an average distance to the future of 6.04 $\pm$ 1.57 AAs and outperformed the naive model at 17 of 23 timepoints (74\%).
After the first three training windows where HI measurements were relatively sparse, the HI antigenic novelty coefficient was stable across all remaining timepoints (Fig.~\ref{fig:unadjusted_model_accuracy_and_coefficients_for_natural_populations}).
In contrast, epitope antigenic novelty estimated a distance of 7.06 $\pm$ 1.37 AAs and only outperformed the naive model at six timepoints (26\%).
Epitope antigenic novelty was also the only metric whose coefficient started at a positive value (1.45 $\pm$ 0.10 on average prior to October 2009) and transitioned to a negative value through the validation period (-0.13 $\pm$ 0.32 on average for October 2009 and after).
...
Indeed, the epitope antigenic novelty model is strongly predictive for the first two validation timepoints until it has to predict to October 2005.
...
We fit a antigenic novelty model to these 25 sites across the complete validation period and dubbed this the ``oracle antigenic novelty'' model, as it benefited from knowledge of the future in its forecasts.
In contrast to the epitope antigenic novelty model, the oracle antigenic novelty model produced a strong positive coefficient across all training windows (1.06 $\pm$ 0.23) and consistently outperformed the original model with an average distance to the future of 5.59 $\pm$ 1.30 AAs (Supplemental Fig.~\ref{sup_fig:unadjusted_composite_model_accuracy_and_coefficients_for_natural_populations_epitope_vs_oracle}).
...
LBI was the best individual fitness metric by average distance to the future (Fig.~\ref{fig:unadjusted_model_accuracy_and_coefficients_for_natural_populations}) and tied HI antigenic novelty and mutational load by outperforming the naive model at 17 (74\%) timepoints (Table~\ref{table_natural_model_selection}).
...
In contrast, the three-metric model strongly preferred the HI antigenic novelty and mutational load metrics over LBI for the majority of the validation period, producing an average LBI coefficient of 0.05 $\pm$ 0.50.
...
However, the dominance of LBI later in this three-metric model also suggests that HI assays may no longer be as predictive as they have been historically.

As with the simulated populations, we validated the performance of the best model for natural populations using estimated and observed clade growth ratios and the ranking of estimated best strains compared to the observed closest strains to future populations.
The composite model of mutational load and LBI effectively captured clade dynamics with a growth correlation of $R^2 = 0.35$ and growth and decline accuracies of 87\% and 89\%, respectively (Supplemental Fig.~\ref{sup_fig:validation_of_best_model_for_natural_populations}A).
...
The composite of HI antigenic novelty, mutational load, and LBI outperformed all other models on the test data with an average distance of 6.12 $\pm$ 1.42 AAs.
The simpler composite of HI antigenic novelty and mutational load performed nearly as well, however, with more stable coefficients per metric over time.
...
The composite model partially captured clade dynamics with a Pearson's correlation of $R^2 = 0.52$ between observed and estimated growth ratios and growth and decline accuracies of 67\% and 63\%, respectively (Fig.~\ref{fig:test_of_best_model_for_natural_populations}A).
...
Similarly, the observed and estimated strain ranks strongly correlated (Spearman's $\rho^2 = 0.69$) across all strains and test timepoints (Fig.~\ref{fig:test_of_best_model_for_natural_populations}D).
...
In addition to three composite models described above, we evaluated a composite of FRA antigenic novelty and mutational load.
Since the earliest FRA data are from 2012, we anticipated that there were enough measurements to fit a model across the test data time interval.
...
None of the three original composite models performed better across the test data than the original best model (Supplemental Fig.~\ref{sup_fig:updated_model_accuracy_and_coefficients_for_natural_populations_across_test_data}).
These results confirmed that the coefficients for our selected best model would be most accurate for live forecasts.
Interestingly, the FRA antigenic novelty metric received a consistently positive coefficient in its composite with mutational load and this composite ranked among the best models for the test data.
These results indicate that this FRA-based composite could eventually replace the HI-based composite with mutational load.
...
\begin{figure*}[ht]
  ...
  \caption{
    Snapshot of the last two years of seasonal influenza H3N2 evolution on nextstrain.org showing the estimated distance per sample to the future population.
    Distance to the future is calculated for each sample as the Hamming distance of HA amino acid sequences to all other circulating strains weighted by the other sample's projected frequencies under the best fitness model (HI antigenic novelty and mutational load).
  }
...
We observed the latter type of overfitting in the context of sites associated with antigenic drift.
...
While this performance confirms previously demonstrated potential for long-term influenza forecasting \cite{Luksza:2014hj}, the average gain from these models over the naive model appears low at 0.96 AAs per year for validation data and 0.66 AAs per year for test data.
...
Second, the average substitution rate for H3N2's HA is 0.0043 substitutions per site per year or an average accumulation of 2.4 AAs per year \cite{}.
...
Models should integrate information from multiple segments of the influenza genome and will need to balance the fitness benefits of evolution in genes such as neuraminidase \cite{Chen:2018kp} with the costs of reassortment \cite{Villa:2017iw}.
The strong performance of a composite FRA antigenic novelty and mutational load model with recent H3N2 populations indicates that FRAs will continue to be important for future forecasting efforts.
...
Prediction of future influenza virus populations are intrinsically limited by the small number of data points available to train and test models.
...
For a model based on any given combination of fitness metrics, we found the fitness coefficients that minimized the weighted Hamming distance between amino acid sequences from the observed future population at time $u = t + \Delta{t}$ and the estimated future population created by projecting frequencies of strains at time $t$ by their estimated fitnesses.
...
To estimate antigenic drift with genetic data, we implemented the antigenic novelty metric originally defined by {\L}uksza and L\"assig \cite{Luksza:2014hj}.
...
We defined the constant $D_{0} = 14$, as in the original definition of antigenic novelty \cite{Luksza:2014hj}.
...
RAN planned experiments and reviewed the manuscript.
