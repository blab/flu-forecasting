...
While there are not enough historical FRA measurements to validate and test a FRA cross-immunity metric, it may be possible to reuse coefficients from the HI cross-immunity models with modern FRA data.
...
These assays reveal the specific residues that confer antigenic escape at specific sites.
...
By releasing our framework as an open source tool based on modern data science standards like tidy data frames, we hope to encourage continued development by the influenza research community.
...
\jhc{Document HA sequences from GISAID (reference acknowledgements table).}

\subsubsection*{Titers}

\jhc{Document HI titers from influenza CCs.}
...
\jhc{Add equation for weighted Hamming distance here.}
