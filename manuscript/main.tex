...
With the original coefficients, our combined HI cross-immunity, mutational load, and LBI model favored the growth of the currently circulating clade A1b/197R, the maintenance of A1b/137F, and the decline of 3c3.A (Fig.~\ref{fig:nextstrain_forecasts}).
...
The best models from this framework effectively capture clade dynamics and accurately identify optimal vaccine candidates without relying on phylogenetic clades as model targets.

Our evaluation of models by time-series cross-validation and true out-of-sample tests reveal substantial potential for overfitting models to historical data.
The clearest examples of this overfitting were found in the definition of antigenic drift by historically relevant epitope sites and the definition of functional constraint by DMS experiments against a single viral background.

Indeed, the modest improvement of our best models over a naive model of no change between years highlights the inherent difficulty and potentially limited ability to make precise forecasts for such a complex, dynamical system.
...
\item{\rnc{The overall gain from prediction is low (1aa), but this has to be seen in the light of the short prediction horizon (1y) and the larger coalescence time of about 3-8y. Hence we expect diversity to persist and any gain has to come from clade frequencies.}}
\item{\rnc{The future tends to anchor deeper in the tree away from the tips. This may be the primary reason for the success of the LBI and non-epitope. Both of these metrics are lower at the tips than they are at viruses close to the base of the tree.}}
...
\item{\jhc{Cite Bell et al. 2019 and ML-based forecasting work for flu}}
\end{itemize}
