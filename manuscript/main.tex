...
Most modern H3N2 strains carry a glycosylation motif that reduces their binding efficiency in HI assays \cite{Chambers:2015jt,Zost2017}.
Correspondingly, many influenza collaborating centers now also measure cross-reactivity by focus reduction assays (FRAs) \cite{Okuno:1990uu}.
...
Here, we provide the first such open source framework for forcasting the genetic composision of future seasonal influenza populations.
...
\begin{figure*}[ht]
  ...
  \caption{
  \rnc{missing panel labels!}
    Schematic representation of the fitness model for simulated H3N2-like populations wherein the fitness of samples at timepoint $t$ determines the estimated frequency of samples with similar sequences one year in the future at timepoint $u$.
...
We estimated the frequency of virus samples every six months using kernel density estimation (KDE) to represent the frequency of each sample.
...
\rnc{wouldn't we expect the true fitness have coefficent 1.0? A plausible reviewer question would be ``how does the model do with coefficient 1.0?"}
...
\begin{figure*}[p]
  ...
  \rnc{Growth rates have units. Is this 1/year? Also what exactly is $\log_{10}$ growth rate? -2 == 0.01/year?}
  ...
  \rnc{it would be interesting to look at this for neg and pos deviations separately.}
...
\rnc{this is not a supplemental figure, right?}
...
\rnc{the p-value us probably not meaningful due to phylogenetic relatiobships.}
...
\rnc{Is the above statement accurate? this refers to the true fitness model which has a next to constant coefficient around 9. Risk of overfitting is minimal and the only measure that is substantially worse is Pearson's $r^2$ which is sensitive to outliers. Furthermore, the clades on which panel A is based are not independent.}
...
\rnc{Just a suggestion:}
Across our training period, HA sequences of H3N2 influenza populations one year apart was 6.40 $\pm$ 1.36 AAs.
...
\rnc{I thought this was predictive until 2008.}
...
These results support our hypothesis that fitness benefit of mutations at the original 49 sites was due to historical contingency and that the success of previous epitope models based on these sites partly due to ``borrowing from the future''.
...
To aid with identification of potential vaccine candidates for the next season, we annotated samples in the phylogeny by their estimated distance to the future based on our best model (Fig.~\ref{fig:nextstrain_distance_to_future}).
...
Even well performing models with few parameters will sometimes fail due to the probabilistic nature of evolution: The model with the best performance across our validation data -- mutational load and LBI -- was also one of the worst models across our test data.
...
Increasing the complexity therefore makes models prone to overfitting.
Across the training and validation period, we found that antigenic drift and mutational load are the most robust predictors of future success for seasonal influenza H3N2 populations.
...
This solution requires both a ``ground distance'' between pairs of samples from both populations and weights assigned to each sample that determine how much that sample contributes to the overall distance.
