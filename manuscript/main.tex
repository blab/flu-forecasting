...
For each model's empirical distribution of earth mover's distances to the future at a specific timepoint $t$, $y(t)$, and the corresponding naive model's distribution, $x(t)$, we calculated the difference between the two models as $y(t) - x(t)$ such that values less than zero represented timepoints when the given model outperformed the naive model.
